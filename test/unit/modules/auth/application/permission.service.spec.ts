...
describe('PermissionService', () => {
  ...
  describe('isAdmin', () => {
    it('should return true if user has admin role', async () => {
      const adminUser: UserModel = {
        ...mockUserModel,
        roles: [{ ...mockUserRole, name: 'admin' }],
      };
      userRepository.findByIdWithRoles.mockResolvedValue(adminUser);

      const result = await service.isAdmin(mockUserId);
  ...
  describe('isManager', () => {
    it('should return true if user has manager role', async () => {
      const managerUser: UserModel = {
        ...mockUserModel,
        roles: [mockManagerRole],
      };
      userRepository.findByIdWithRoles.mockResolvedValue(managerUser);

      const result = await service.isManager(mockUserId);

      expect(result).toBe(true);
    });

    it('should return false if user does not have manager role', async () => {
      userRepository.findByIdWithRoles.mockResolvedValue(mockUserModel);
  ...
  describe('isUser', () => {
    it('should return true if user has user role', async () => {
      userRepository.findByIdWithRoles.mockResolvedValue(mockUserModel);

      const result = await service.isUser(mockUserId);

      expect(result).toBe(true);
    });

    it('should return false if user does not have user role', async () => {
      const adminUser: UserModel = {
        ...mockUserModel,
        roles: [{ ...mockUserRole, name: 'admin' }],
      };
